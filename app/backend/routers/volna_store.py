"""VOLNA — alcohol store catalog, orders and settings."""
...
from services.volna_orders import Volna_ordersService
...
from services.telegram import notify_volna_order, notify_volna_status_change
...
router = APIRouter(
    prefix="/api/v1/volna",
    tags=["volna"],
    dependencies=[Depends(require_module("volna"))],
)
...
def _require_admin(request: Request) -> None:
    require_store_partner_or_admin(request, "volna")
...
async def list_categories(db: AsyncSession = Depends(get_db)):
    svc = Volna_categoriesService(db)
...
async def update_order_status(order_id: int, status: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if old_status != status:
        await notify_volna_status_change({
            "order_id": obj.id,
        ...
        try:
            ...
            await notify_store_order_status(db, store_type="volna", order=obj, new_status=status)
        except Exception:
            pass
    return _serialize(obj)
