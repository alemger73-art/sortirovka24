...
class DatabaseManager:
    ...
    # Parameters that asyncpg does not accept via URL query string.
    # Note: "ssl" IS supported by asyncpg natively (e.g. ssl=require) and must NOT be stripped.
    # asyncpg's native ssl=require handling correctly enables SNI (Server Name Indication),
    # which is required by Neon serverless PostgreSQL for endpoint routing.
    ...
    def _normalize_async_database_url(self, raw_url: str) -> str:
        ...
        It also strips query-string parameters that asyncpg does not understand
        (e.g. ``channel_binding``, ``sslmode``), which some managed PostgreSQL
        providers inject automatically.
        """
        try:
        ...
        if "+aiosqlite" in drivername or "+asyncpg" in drivername or "+aiomysql" in drivername:
            ...
            # Still need to strip unsupported params for asyncpg
            if "+asyncpg" in drivername:
                url = self._strip_unsupported_asyncpg_params(url)
            # CRITICAL: Use render_as_string(hide_password=False) instead of str(url).
            # In SQLAlchemy 2.x, str(url) masks the password with '***', which causes
            # "password authentication failed" errors when the masked URL is used to connect.
    ...
    def _strip_unsupported_asyncpg_params(self, url):
        """Remove query-string parameters that asyncpg's connect() does not accept."""
        if not url.query:
            return url
        cleaned = {k: v for k, v in url.query.items() if k not in self._ASYNCPG_UNSUPPORTED_PARAMS}
        stripped_keys = set(url.query.keys()) - set(cleaned.keys())
        if stripped_keys:
            logger.warning(
                f"Stripped unsupported asyncpg connection params: {stripped_keys}"
            )
        return url.set(query=cleaned)
    ...
    async def _test_connection(self, engine) -> bool:
        """Test if the engine can actually connect to the database."""
        try:
    ...
    async def init_db(self):
        """Initialize database connection with thread safety and retry logic.
        
        The entire initialization is protected by _init_lock to prevent
        race conditions where multiple callers see engine=None and all
        attempt to initialize concurrently.
        
        Includes retry logic to handle Neon serverless cold starts where
        the first connection attempt may fail with password authentication errors.
        """
    ...
    async def create_tables(self):
        ...
        try:
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    logger.info(f"🔧 Table creation attempt {attempt}/{max_retries}...")
                    ...
                    logger.warning(f"Table creation attempt {attempt}/{max_retries} failed: {e}")
                    if attempt < max_retries:
                        logger.info(f"Retrying in {retry_delay} seconds...")
                        await asyncio.sleep(retry_delay)
    ...
    async def ensure_initialized(self):
        ...
        Uses double-checked locking pattern with retry support for Neon cold starts.
        """
        ...
        # init_db() has its own internal lock, so we don't need to hold _init_lock here
        try:
            await self.init_db()
