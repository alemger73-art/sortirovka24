...
# Load .env before other modules read os.environ (local dev + Docker with mounted .env)
try:
...
from services.bucket_init import initialize_buckets
# MODULE_IMPORTS_END
...
async def _run_startup_initialization():
    ...
    if db_ready:
        ...
        try:
            from routers.partner_auth import initialize_all_partner_credentials

            await initialize_all_partner_credentials()
        except Exception as e:
            logger.warning(f"Partner credentials initialization failed: {e}")
