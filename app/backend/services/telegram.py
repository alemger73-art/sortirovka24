...
async def notify_food_order(data: dict) -> bool:
    ...
    items_text = "\n".join(lines) if lines else "—"
    restaurant = _escape_html(data.get("restaurant_name") or "Алем Фуд")
...
async def notify_pharmacy_order(data: dict) -> bool:
    """Send notification about a new АПТЕКА (pharmacy) order."""
    items = data.get("items") or []
    if isinstance(items, str):
        try:
            import json
    ...
    for idx, item in enumerate(items, 1):
        ...
        weight = item.get("weight", "")
        weight_part = f" ({weight})" if weight else ""
        rx_mark = " 💊Rx" if item.get("rx") else ""
        if item.get("is_gift"):
            lines.append(f"🎁 {_escape_html(name)}{weight_part}")
        else:
            lines.append(f"{idx}. {_escape_html(name)}{weight_part}{rx_mark} ×{qty} = {sum_val} ₸")
    ...
    rx_line = ""
    if data.get("has_rx"):
        rx_line = "\n⚠️ <b>В заказе есть рецептурные препараты — нужен рецепт при выдаче.</b>"
...
async def notify_pharmacy_status_change(data: dict) -> bool:
    """Notify Telegram when an АПТЕКА order status changes."""
    status_map = {
        "new": "Новый",
        "processing": "В работе",
        "delivered": "Доставлен",
    ...
    new_label = status_map.get(data.get("new_status", ""), data.get("new_status", "—"))
    text = (
        "📦 <b>Изменение статуса заказа АПТЕКА</b>\n\n"
        f"<b>№ заказа:</b> {data.get('order_id', '—')}\n"
        f"<b>Клиент:</b> {_escape_html(data.get('customer_name', '—'))}\n"
        f"<b>Телефон:</b> {_escape_html(data.get('customer_phone', '—'))}\n"
        f"<b>Было:</b> {_escape_html(old_label)}\n"
