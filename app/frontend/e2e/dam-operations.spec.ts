...
test('cancellation requires reason and settings hide token after save',async({page})=>{const s=await setup(page);await page.goto('/partner/dam-alem?section=orders&order=71');await page.getByRole('button',{name:'Отменить заказ',exact:true}).click();await expect(page.getByRole('button',{name:'Подтвердить отмену'})).toBeDisabled();await page.getByLabel('Причина отмены',{exact:true}).fill('Клиент передумал');await page.getByRole('button',{name:'Подтвердить отмену'}).click();expect(s.order.cancellation_reason).toBe('Клиент передумал');await page.getByRole('button',{name:'Telegram',exact:true}).click();await page.getByLabel('Токен бота',{exact:true}).fill('123456:fake-test-token');await page.getByLabel('Канал или группа',{exact:true}).fill('-100123456');await page.getByRole('button',{name:'Сохранить настройки',exact:true}).click();await expect(page.getByLabel('Токен бота',{exact:true})).toHaveValue('');await expect(page.getByRole('status')).toContainText('Настройки сохранены');expect(s.settings.enabled).toBe(false);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);});
