...
export default function AdminPharmacy({ partnerMode = false }: AdminPharmacyProps) {
  ...
  const [settings, setSettings] = useState<PharmacySettings>({} as PharmacySettings);
  const [editingCat, setEditingCat] = useState<Partial<PharmacyCategory> | null>(null);
  const [editingProduct, setEditingProduct] = useState<Partial<PharmacyProduct> | null>(null);
  ...
  function openCreateProduct() {
    setEditingProduct({ is_active: true, is_popular: false, in_stock: true, requires_prescription: false, sort_order: products.length + 1 });
    setProductDialogOpen(true);
  }

  function openEditProduct(product: PharmacyProduct) {
  ...
  function openEditCategory(category: PharmacyCategory) {
  ...
  async function handleSaveProduct() {
    ...
    try {
      await savePharmacyProduct({
        ...
        old_price: editingProduct.old_price ? Number(editingProduct.old_price) : undefined,
        ...
        {tabs.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              section === id ? 'bg-teal-50 text-teal-700 border border-teal-200' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
            }`}
          >
              ...
              {ORDER_FILTERS.map((f) => (
                <button
                  ...
                  className={`shrink-0 px-3 py-2 rounded-lg text-sm font-medium transition-colors min-h-[40px] ${
                    orderFilter === f.id ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
                >
                  {f.label}
                ...
                <div className="text-sm space-y-0.5">
                  {productItems.map((it, i) => (
                    <div key={i} className="flex justify-between gap-2 text-gray-600">
                      <span className="min-w-0 truncate">{it.name} ×{it.qty}{it.rx ? ' 💊Rx' : ''}</span>
                    ...
                    {(['new', 'processing', 'delivered', 'cancelled'] as const).map((s) => (
                      <button
                        ...
                        className={`text-xs px-3 py-2.5 rounded-lg min-h-[40px] ${
                          o.status === s ? 'bg-teal-100 text-teal-700 font-medium' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                        }`}
                      >
                        {ORDER_STATUS[s]}
                      </button>
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center gap-0.5 py-2 px-1 min-h-[56px] transition-colors ${
                  isActive ? 'text-teal-700' : 'text-gray-500 active:bg-gray-50'
                }`}
              >
