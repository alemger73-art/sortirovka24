...
export default function DamDeliveries({ openOrder }: { openOrder: (id: number) => void }) {
  ...
  return <section className="space-y-4 min-w-0">
    ...
    <p className="text-sm text-muted-foreground">{t('cabinet.deliveryHelp')}</p>
    ...
    <aside className="rounded-xl border p-4 text-sm space-y-2">
      <Link className="font-semibold underline" to="/cabinet/courier" target="_blank" rel="noopener noreferrer">{t('cabinet.courier')}</Link>
      <p className="text-muted-foreground">{t('cabinet.courierAccess')}</p>
    </aside>
  </section>;
}
