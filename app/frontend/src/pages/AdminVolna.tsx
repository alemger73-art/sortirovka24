...
function getORDER_STATUS(adminT: (key: string) => string) {
  const ORDER_STATUS: Record<string, string> = { new: adminT("admin.ui.0508"), processing: adminT("admin.ui.0128"), delivered: adminT("admin.ui.0513"), cancelled: adminT("admin.ui.0514") };
  return ORDER_STATUS;
}

function formatOrderDate(raw: string, locale: string) {
  if (!raw) return '—';
  try { return new Date(raw).toLocaleString(locale, { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }); } catch { return raw; }
}
...
export default function AdminVolna({ partnerMode = false }: AdminVolnaProps) {
  ...
  async function loadAll(fullScreenLoader = false) {
    ...
    try {
      ...
      console.error(e);
      toast.error(adminT("admin.ui.1236"));
    } finally {
  ...
  async function handleSaveCategory() {
    ...
    try {
      ...
      toast.success(adminT("admin.ui.0482")); setCategoryDialogOpen(false); setEditingCat(null); await loadAll();
  ...
  async function handleSaveProduct() {
    ...
    try {
      ...
      toast.success(adminT("admin.ui.0720")); setProductDialogOpen(false); setEditingProduct(null); await loadAll();
    } catch { toast.error(adminT("admin.ui.0055")); }
  }
  ...
  async function handleSaveSettings() {
    try {
      ...
      setSettings(saved); toast.success(adminT("admin.ui.1237"));
    } catch { toast.error(adminT("admin.ui.0722")); }
  }

  async function handleOrderStatus(orderId: number, status: string) {
    try { await updateVolnaOrderStatus(orderId, status); toast.success(adminT("admin.ui.0047")); await loadAll(); } catch { toast.error(adminT("admin.ui.0486")); }
  }

  const tabs = [
    { id: 'products' as Section, label: adminT("admin.ui.0698"), icon: Package },
    { id: 'categories' as Section, label: adminT("admin.ui.0242"), icon: FolderTree },
    { id: 'orders' as Section, label: adminT("admin.ui.0239"), icon: ShoppingBag },
    { id: 'delivery' as Section, label: adminT("admin.ui.0762"), icon: Map },
    { id: 'gifts' as Section, label: adminT("admin.ui.0608"), icon: Gift },
    { id: 'settings' as Section, label: adminT("admin.ui.0245"), icon: Settings },
  ];

  if (loading) return <div className="p-8 text-center text-gray-400">{adminT("admin.ui.1238")}</div>;
    ...
    <div className="space-y-4 pb-20 md:pb-0">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">{adminT("admin.ui.1239")}</p>
        </div>
        <a href="/volna" target="_blank" rel="noopener noreferrer" className="text-sm text-violet-600 hover:underline">{adminT("admin.ui.0733")}</a>
      </div>
      ...
      {section === 'products' && (
        <div className="space-y-4">
          <Button size="sm" onClick={() => { setEditingProduct({ is_active: true, is_popular: false, sort_order: products.length + 1 }); setProductDialogOpen(true); }}><Plus className="h-4 w-4 mr-1" /> {adminT("admin.ui.0737")}</Button>
              ...
              <div key={p.id} className="bg-white border rounded-xl overflow-hidden">
                ...
                <div className="p-3">
                  ...
                  <div className="flex gap-1 mt-2">
                    ...
                    <Button size="sm" variant="outline" className="text-red-600" onClick={async () => { if (confirm(adminT("admin.ui.1028"))) { await deleteVolnaProduct(p.id); await loadAll(); } }}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </div>
              </div>
            ))}
      ...
      {section === 'categories' && (
        <div className="space-y-2">
          <Button size="sm" onClick={() => { setEditingCat({ is_active: true, sort_order: categories.length + 1 }); setCategoryDialogOpen(true); }}><Plus className="h-4 w-4 mr-1" /> {adminT("admin.ui.0231")}</Button>
          {categories.map(c => (
            <div key={c.id} className="flex items-center gap-3 bg-white border rounded-xl p-3">
              ...
              <Button size="sm" variant="outline" className="text-red-600" onClick={async () => { if (confirm(adminT("admin.ui.1028"))) { await deleteVolnaCategory(c.id); await loadAll(); } }}><Trash2 className="h-4 w-4" /></Button>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="space-y-3">
          {filteredOrders.map(o => (
            <div key={o.id} className="bg-white border rounded-xl p-4 space-y-2">
              ...
              <p className="font-bold text-violet-700">{Math.round(o.total_amount).toLocaleString(adminLocale)} ₸</p>
      ...
      {section === 'delivery' && (
        <div className="space-y-4">
          ...
          <Button onClick={() => void handleSaveSettings()} className="bg-violet-600 hover:bg-violet-700 text-white"><Save className="h-4 w-4 mr-1" /> {adminT("admin.ui.0748")}</Button>
        </div>
      )}
      ...
      {section === 'gifts' && (
        <div className="space-y-4">
          ...
          <Button onClick={async () => { await saveVolnaSettings({ ...settings, loyalty_enabled: loyaltyEnabled ? '1' : '0', loyalty_gifts: serializeLoyaltyGifts(loyaltyGifts) } as Record<string, string>); toast.success(adminT("admin.ui.0726")); }} className="bg-violet-600 hover:bg-violet-700 text-white">{adminT("admin.ui.0750")}</Button>
        </div>
      )}
      ...
      {section === 'settings' && (
        ...
        <div className="bg-white border rounded-xl p-4 space-y-4 max-w-lg">
          ...
          {[['store_name', adminT("admin.ui.0213")], ['store_tagline', adminT("admin.ui.0120")], ['hero_title', adminT("admin.ui.0119")], ['promo_title', adminT("admin.ui.1240")], ['promo_subtitle', adminT("admin.ui.1241")], ['promo2_title', adminT("admin.ui.1242")], ['promo2_subtitle', adminT("admin.ui.1243")], ['min_order', adminT("admin.ui.1244")], ['store_phone', adminT("admin.ui.0466")]].map(([key, label]) => (
          ...
          <Button onClick={() => void handleSaveSettings()} className="bg-violet-600 hover:bg-violet-700 w-full text-white"><Save className="h-4 w-4 mr-1" /> {adminT("admin.ui.0096")}</Button>
      ...
      <Dialog open={productDialogOpen} onOpenChange={setProductDialogOpen}>
        <DialogContent className={MOBILE_DIALOG}>
          <DialogHeader><DialogTitle>{editingProduct?.id ? adminT("admin.ui.1245") : adminT("admin.ui.0765")}</DialogTitle></DialogHeader>
          {editingProduct && (
            <div className="space-y-3">
              ...
              <Input placeholder={adminT("admin.ui.1246")} value={editingProduct.weight || ''} onChange={(e) => setEditingProduct(p => p ? ({ ...p, weight: e.target.value }) : p)} />
              ...
              <Button onClick={() => void handleSaveProduct()} className="bg-violet-600 w-full text-white">{adminT("admin.ui.0096")}</Button>
            </div>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={categoryDialogOpen} onOpenChange={setCategoryDialogOpen}>
        <DialogContent className={MOBILE_DIALOG}>
          <DialogHeader><DialogTitle>{adminT("admin.ui.0231")}</DialogTitle></DialogHeader>
          {editingCat && (
            <div className="space-y-3">
              ...
              <Button onClick={() => void handleSaveCategory()} className="bg-violet-600 w-full text-white">{adminT("admin.ui.0096")}</Button>
            </div>
          )}
        </DialogContent>
      </Dialog>
