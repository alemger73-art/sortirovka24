...
export default function DamAlemOrders() {
  ...
  const target = order?.delivery_method === 'delivery' && ['ready', 'in_progress'].includes(order?.status || '') ? '' : order?.status === 'ready' && order.delivery_method !== 'delivery' ? 'done' : next[order?.status || ''];
  return <div className="space-y-5 min-w-0">
    <div className="flex flex-wrap justify-between gap-3"><div><h3 className="text-xl font-bold">{adminT('admin.dam.final.138')}</h3><p className="text-sm text-muted-foreground">{adminT('admin.dam.final.139')} {lastLoaded ? adminT('admin.dam.final.140').replace('{0}', () => String(new Date(lastLoaded).toLocaleTimeString(locale))) : adminT('admin.dam.final.141')}</p></div><Button variant="outline" onClick={() => void load()}>{adminT('admin.dam.final.142')}</Button></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(260px,1fr)_minmax(0,1.6fr)]">
      <section className="min-w-0 space-y-2" aria-label={adminT('admin.dam.final.150')}>
        ...
        {rows.map(o => <button key={o.id} disabled={busy} onClick={() => { const p = new URLSearchParams(params); p.set('section', 'orders'); p.set('order', String(o.id)); setParams(p); }} className={`w-full rounded-xl border p-4 text-left ${selected === o.id ? 'border-red-400 bg-red-50' : 'bg-card hover:bg-gray-50'}`}><span className="flex flex-wrap justify-between gap-2"><strong>№{o.id} · {money(o.total_amount)}</strong><span className="text-xs rounded-full bg-gray-100 px-2 py-1">{orderLabels[o.status] || o.status}</span></span><span className="block mt-2 break-words">{o.customer_name || adminT('admin.dam.final.024')} · {o.delivery_method === 'dine_in' ? adminT('workflow.onsite') : o.delivery_method === 'pickup' ? adminT('admin.dam.final.025') : adminT('admin.dam.final.026')}</span><span className="text-xs text-muted-foreground">{date(o.created_at)}</span></button>)}
      ...
      <section className="min-w-0 rounded-2xl border bg-card p-4 sm:p-6 space-y-4" aria-label={adminT('admin.dam.final.156')}>
        {!selected && <p className="text-muted-foreground">{adminT('admin.dam.final.157')}</p>}
        ...
        {order && <><div className="flex flex-wrap gap-3 justify-between"><h3 className="font-bold text-xl">{adminT('admin.dam.final.159')}{order.id}</h3><Button variant="outline" disabled={busy || editing || cancelOpen} onClick={() => { void refreshDetail().catch(e => toast.error(e.message)); }}>{adminT('admin.dam.final.160')}</Button></div>
          ...
          {order.delivery_method === 'delivery' && order.status === 'ready' && <p className="rounded-xl border p-3 text-sm">{adminT('workflow.courierNext')}</p>}
          ...
          {!closed && <div className="flex flex-wrap gap-2">{target && <Button disabled={busy} onClick={() => void change({ status: target })}>{target === 'done' ? order.delivery_method !== 'delivery' ? adminT('admin.dam.final.167') : adminT('admin.dam.final.168') : orderLabels[target]}</Button>}{order.payment_status !== 'paid' && <Button variant="outline" disabled={busy} onClick={() => void change({ payment_status: 'paid' })}>{adminT('admin.dam.final.169')}</Button>}<Button variant="outline" disabled={busy} onClick={() => setCancelOpen(!cancelOpen)}>{adminT('admin.dam.final.170')}</Button></div>}
          ...
          <Button variant="outline" disabled={busy} onClick={() => { setEditing(!editing); setNote(order.operator_note || ''); setAddress(order.delivery_address || ''); }}>{adminT('admin.dam.final.176')}{!closed && order.delivery_method === 'delivery' ? adminT('admin.dam.final.177') : ''}</Button>
          {editing && <fieldset disabled={busy} className="space-y-3"><label className="block">{adminT('admin.dam.final.178')}<textarea aria-label={adminT('admin.dam.final.178')} maxLength={2000} className="w-full rounded-lg border p-3" value={note} onChange={e => setNote(e.target.value)} /></label>{!closed && order.delivery_method === 'delivery' && <label className="block">{adminT('admin.dam.final.179')}<Input value={address} maxLength={1000} onChange={e => setAddress(e.target.value)} /></label>}<Button onClick={() => void change({ operator_note: note, ...(!closed && order.delivery_method === 'delivery' ? { delivery_address: address } : {}) })}>{adminT('admin.dam.final.180')}</Button></fieldset>}
