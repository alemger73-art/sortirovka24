...
const TO_EXAMPLES = ['Центр Караганды', 'Ж/Д вокзал', 'Больница'];
...
export default function Taxi() {
  ...
  const resolvePoint = useCallback(async (kind: PointKind, address?: string, gps?: boolean) => {
  ...
  const calculateQuote = useCallback(async () => {
    if (!fromCoords || !toCoords) {
      toast.error('Укажите точки отправления и назначения');
      return;
    }
    setQuoting(true);
    try {
          ...
          <div className="rounded-3xl bg-white shadow-2xl shadow-black/30 overflow-hidden">
            ...
            <div className="p-5 md:p-6 space-y-5">
              <AddressField
                ...
                onGps={() => resolvePoint('from', undefined, true)}
                onSearch={() => resolvePoint('from', fromAddress)}
              ...
              <AddressField
                ...
                onGps={() => resolvePoint('to', undefined, true)}
                onSearch={() => resolvePoint('to', toAddress)}
