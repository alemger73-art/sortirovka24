...
export default function CourierHub() {
  ...
  async function submit() {
    ...
    if (!form.full_name.trim()) {
      toast.error(publicT("public.CourierHub.text141"));
      return;
    }
    if (!form.phone.trim()) {
      toast.error(publicT("public.CourierHub.text142"));
      return;
    }
    if (!form.photo_url || !form.id_photo_url) {
      toast.error(publicT("public.CourierHub.text143"));
      return;
    }
    if (form.vehicle_type !== 'foot' && !form.vehicle_photo_url) {
      toast.error(publicT("public.CourierHub.text144"));
      return;
    }
      ...
      <div className="min-h-screen bg-gray-900">
        <div className="bg-gradient-to-br from-orange-400 to-amber-500 px-4 py-10 md:py-14">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="text-3xl md:text-4xl font-black text-gray-900">{publicT("public.CourierHub.text147")}</h1>
            <p className="mt-2 text-gray-800 font-medium">{publicT("public.CourierHub.text148")}</p>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-3xl px-4 py-8 space-y-6 -mt-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              { icon: Wallet, title: publicT("public.CourierHub.text149"), desc: publicT("public.CourierHub.text150") },
              { icon: MapPin, title: publicT("public.CourierHub.text151"), desc: publicT("public.CourierHub.text152") },
              { icon: Shield, title: publicT("public.CourierHub.text153"), desc: publicT("public.CourierHub.text154") },
          ...
          {loading ? (
            ...
            <div className="rounded-3xl bg-white p-8 text-center shadow-xl">
              ...
              <h2 className="text-xl font-bold text-gray-900">{publicT("public.CourierHub.text155")}</h2>
              <p className="text-gray-500 mt-2 mb-6">{publicT("public.CourierHub.text156")}</p>
              <Button
                className="h-12 px-8 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-bold"
                onClick={() => navigate('/cabinet/courier')}
              >
                {publicT("public.CourierHub.text157")} </Button>
            </div>
              ...
              {!getAccountToken() ? (
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <Input
                      placeholder={publicT("public.CourierHub.text166")}
                      value={form.full_name}
                      onChange={(e) => setForm({ ...form, full_name: e.target.value })}
                      className="rounded-xl h-11"
                    />
                    <Input
                      placeholder={publicT("realestate.form.phone")}
                      value={form.phone}
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-800 mb-2">{publicT("public.CourierHub.text167")}</p>
                  ...
                  {form.vehicle_type === 'car' && (
                    <Input
                      placeholder={publicT("public.CourierHub.text168")}
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-800 mb-2">{publicT("public.CourierHub.text170")}</p>
                    <CourierDocUpload
          ...
          <p className="text-center text-white/40 text-sm">
            {publicT("public.CourierHub.text171")} <Link to="/food" className="text-orange-300 underline">DAM ALEM 2.0</Link>
            {' · '}
            {publicT("public.CourierHub.text172")} <Link to="/taxi/driver" className="text-orange-300 underline">{publicT("public.CourierHub.text173")}</Link>
          </p>
        </div>
      </div>
    </Layout>
