...
export default function PlatformCartDrawer({ open, onClose }: Props) {
  ...
  function handleClear(segId: string, label: string) {
    if (!window.confirm(st('Очистить корзину «{0}»? Все товары будут удалены.', [label]))) return;
    clearPlatformCartSegment(segId);
    setSegments(getPlatformCartSegments());
    toast.success(st('Корзина «{0}» очищена', [label]));
  }
  ...
  return (
    <div className="fixed inset-0 z-[80]">
      <button type="button" className="absolute inset-0 bg-black/40" aria-label={st('Закрыть')} onClick={onClose} />
      <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white dark:bg-gray-950 shadow-2xl flex flex-col">
        <div className="flex items-center justify-between border-b px-4 py-4 dark:border-gray-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-lg">{st('Мои корзины')}</h2>
          </div>
          <button type="button" onClick={onClose} aria-label={st('Закрыть')} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {segments.length === 0 ? (
            <p className="text-center text-gray-500 py-12">{st('Корзины пусты')}</p>
          ) : (
            segments.map((seg) => {
              const label = seg.id === 'pharmacy' ? st('Аптека') : seg.label;
              return (
                <div
                  key={seg.id}
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-500">{seg.count} {seg.count === 1 ? st('товар') : seg.count < 5 ? st('товара') : st('товаров')}</p>
                    </div>
                  </div>
                  <div className="flex shrink-0 items-center gap-1">
                    <button
                      type="button"
                      onClick={() => handleClear(seg.id, label)}
                      className="p-2 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40"
                      aria-label={st('Очистить корзину {0}', [label])}
                      title={st('Очистить корзину')}
                    >
        ...
        {total > 0 && (
          <div className="border-t p-4 text-sm text-gray-500 dark:border-gray-800">
            {st('Всего позиций:')} <span className="font-bold text-gray-900 dark:text-white">{total}</span>
            <p className="text-xs mt-1">{st('Каждый магазин оформляется отдельно — своя доставка и оплата.')}</p>
          </div>
        )}
      </div>
    </div>
...
export function PlatformCartButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button
      ...
      title={st('Мои корзины')}
      aria-label={st('Мои корзины')}
    >
      <ShoppingBag className="h-4 w-4" />
      <span className="hidden sm:inline">{st('Корзины')}</span>
