...
type Quote = {items: {name: string; quantity: number; sum: number}[]; total_amount: number; previous_total?: number; paid_amount?: number; amount_due?: number; refund_due?: number; gift_choices?: {id: string; title: string}[]; gift_required?: boolean};
...
export default function OrderReceiptEditor({order, onClose, onSaved}: {order?: OperatorOrder; onClose: () => void; onSaved: (id: number) => void}) {
  ...
  const [giftChoices, setGiftChoices] = useState<{id: string; title: string}[]>([]);
  ...
  const body = () => ({selected_gift_id: giftId || undefined, items: lines.map(x => ({line_index: x.line_index, id: typeof x.id === 'number' ? x.id : undefined, quantity: x.quantity, modifiers: x.modifiers || []})),
  ...
  async function submit(save: boolean) {
    ...
    try {
      ...
      if (save) {
        ...
        const result = await foodOperations<Quote>(path + '/quote', 'POST', body());
        setQuote(result); setGiftChoices(result.gift_choices || []);
      }
  ...
  return <Dialog open onOpenChange={open => {if (!open && !busy) onClose();}}><DialogContent className="max-w-3xl max-h-[90dvh] overflow-y-auto"><DialogHeader><DialogTitle>{t(order ? 'workflow.edit' : 'workflow.manual')}</DialogTitle></DialogHeader>
    <fieldset disabled={busy} className="space-y-4 min-w-0">
      ...
      {giftChoices.length > 0 && <label className="block">{t('workflow.gift')}<select className="w-full border rounded-lg p-2 bg-background" value={giftId} onChange={e => {setGiftId(e.target.value); invalidate();}}><option value="">{t('workflow.chooseGift')}</option>{giftChoices.map(g => <option key={g.id} value={g.id}>{g.title}</option>)}</select></label>}
      {error && <p role="alert" className="rounded-xl bg-red-50 text-red-800 dark:bg-red-950 dark:text-red-200 p-3">{error}</p>}
