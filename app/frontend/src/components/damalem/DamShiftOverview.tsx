...
export default function DamShiftOverview() {
  ...
  return <section className="space-y-4 rounded-2xl border bg-card p-4 md:p-5">
    ...
    {!shifts.length ? <p className="text-sm text-muted-foreground">{t('dam.shift.empty')}</p> : <div className="grid gap-2 md:grid-cols-2">{shifts.map(s => <button key={s.id} onClick={() => setSelected(s.id)} className={`rounded-xl border p-3 text-left transition ${selected === s.id ? 'border-red-400 bg-red-50 dark:bg-red-950/20' : 'hover:bg-muted'}`}><div className="flex items-center justify-between gap-2"><strong>{s.staff_name}</strong><span className={`rounded-full px-2 py-1 text-xs font-bold ${s.active ? 'bg-emerald-100 text-emerald-800' : 'bg-muted text-muted-foreground'}`}>{s.active ? t('dam.shift.active') : t('dam.shift.finished')}</span></div><p className="mt-1 text-xs text-muted-foreground">{new Date(s.opened_at).toLocaleString()} {s.closed_at ? `→ ${new Date(s.closed_at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}` : ''} {s.duration_seconds != null ? `· ${duration(s.duration_seconds)}` : ''}</p></button>)}</div>}
    {selected && <div><h4 className="mb-2 flex items-center gap-2 font-semibold"><Activity className="h-4 w-4" />{t('dam.shift.actions')}</h4>{!actions.length ? <p className="text-sm text-muted-foreground">{t('dam.shift.noActions')}</p> : <div className="max-h-64 space-y-2 overflow-y-auto">{actions.map(a => <div key={a.id} className="flex justify-between gap-3 border-b py-2 text-sm"><span><strong>{a.staff_name}</strong> · {t(actionKey[a.action] || 'dam.shift.actionOther')}{a.entity_id ? ` №${a.entity_id}` : ''}</span><time className="shrink-0 text-xs text-muted-foreground">{new Date(a.created_at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</time></div>)}</div>}</div>}
  </section>;
}
